...
/**
 * @tc.name: GetUsbDeviceInfo_DiskNotExist_TestCase_001
 * @tc.desc: diskId不存在于diskMap_返回E_NON_EXIST
 * @tc.type: FUNC
 * @tc.require: NA
 */
HWTEST_F(DiskManagerTest, GetUsbDeviceInfo_DiskNotExist_TestCase_001, TestSize.Level0)
{
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_DiskNotExist_TestCase_001 Start";
    auto &dm = DiskManager::GetInstance();
    std::string usbInfo;
    EXPECT_EQ(dm.GetUsbDeviceInfo("disk-nonexist-999", usbInfo), E_NON_EXIST);
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_DiskNotExist_TestCase_001 End";
}
 
/**
 * @tc.name: GetUsbDeviceInfo_EmptyExtraInfo_TestCase_002
 * @tc.desc: disk存在但extraInfo为空返回E_OK且usbDeviceInfo为"{}"
 * @tc.type: FUNC
 * @tc.require: NA
 */
HWTEST_F(DiskManagerTest, GetUsbDeviceInfo_EmptyExtraInfo_TestCase_002, TestSize.Level0)
{
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_EmptyExtraInfo_TestCase_002 Start";
    auto &dm = DiskManager::GetInstance();
    Disk disk("disk-8-200", SIZE, "/dev/block/disk-8-200", USB_FLAG);
    dm.OnDiskCreated(disk);
 
    std::string usbInfo;
    EXPECT_EQ(dm.GetUsbDeviceInfo("disk-8-200", usbInfo), E_OK);
    EXPECT_EQ(usbInfo, "{}");
 
    dm.OnDiskDestroyed("disk-8-200");
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_EmptyExtraInfo_TestCase_002 End";
}
 
/**
 * @tc.name: GetUsbDeviceInfo_InvalidJson_TestCase_003
 * @tc.desc: extraInfo为非法JSON返回E_OK且usbDeviceInfo为"{}"
 * @tc.type: FUNC
 * @tc.require: NA
 */
HWTEST_F(DiskManagerTest, GetUsbDeviceInfo_InvalidJson_TestCase_003, TestSize.Level0)
{
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_InvalidJson_TestCase_003 Start";
    auto &dm = DiskManager::GetInstance();
    Disk disk("disk-8-201", SIZE, "/dev/block/disk-8-201", USB_FLAG);
    disk.SetExtraInfo("{bad json");
    dm.OnDiskCreated(disk);
 
    std::string usbInfo;
    EXPECT_EQ(dm.GetUsbDeviceInfo("disk-8-201", usbInfo), E_OK);
    EXPECT_EQ(usbInfo, "{}");
 
    dm.OnDiskDestroyed("disk-8-201");
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_InvalidJson_TestCase_003 End";
}
 
/**
 * @tc.name: GetUsbDeviceInfo_NoUsbFields_TestCase_004
 * @tc.desc: extraInfo JSON无vid/pid等USB字段返回E_OK且usbDeviceInfo为"{}"
 * @tc.type: FUNC
 * @tc.require: NA
 */
HWTEST_F(DiskManagerTest, GetUsbDeviceInfo_NoUsbFields_TestCase_004, TestSize.Level0)
{
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_NoUsbFields_TestCase_004 Start";
    auto &dm = DiskManager::GetInstance();
    Disk disk("disk-8-202", SIZE, "/dev/block/disk-8-202", USB_FLAG);
    disk.SetExtraInfo(R"({"ODD_INFO":{"DISC_TYPE":"CD-R"}})");
    dm.OnDiskCreated(disk);
 
    std::string usbInfo;
    EXPECT_EQ(dm.GetUsbDeviceInfo("disk-8-202", usbInfo), E_OK);
    EXPECT_EQ(usbInfo, "{}");
 
    dm.OnDiskDestroyed("disk-8-202");
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_NoUsbFields_TestCase_004 End";
}
 
/**
 * @tc.name: GetUsbDeviceInfo_PartialUsbFields_TestCase_005
 * @tc.desc: extraInfo仅有部分USB字段返回E_OK且仅含已有字段
 * @tc.type: FUNC
 * @tc.require: NA
 */
HWTEST_F(DiskManagerTest, GetUsbDeviceInfo_PartialUsbFields_TestCase_005, TestSize.Level0)
{
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_PartialUsbFields_TestCase_005 Start";
    auto &dm = DiskManager::GetInstance();
    Disk disk("disk-8-203", SIZE, "/dev/block/disk-8-203", USB_FLAG);
    disk.SetExtraInfo(R"({"vid":"0781","pid":"5581"})");
    dm.OnDiskCreated(disk);
 
    std::string usbInfo;
    EXPECT_EQ(dm.GetUsbDeviceInfo("disk-8-203", usbInfo), E_OK);
    EXPECT_NE(usbInfo.find("\"vid\":\"0781\""), std::string::npos);
    EXPECT_NE(usbInfo.find("\"pid\":\"5581\""), std::string::npos);
    EXPECT_EQ(usbInfo.find("serialNumber"), std::string::npos);
 
    dm.OnDiskDestroyed("disk-8-203");
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_PartialUsbFields_TestCase_005 End";
}
 
/**
 * @tc.name: GetUsbDeviceInfo_Success_TestCase_006
 * @tc.desc: extraInfo包含完整vid/pid/serialNumber/busnum/devnum返回E_OK且全部提取
 * @tc.type: FUNC
 * @tc.require: NA
 */
HWTEST_F(DiskManagerTest, GetUsbDeviceInfo_Success_TestCase_006, TestSize.Level0)
{
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_Success_TestCase_006 Start";
    auto &dm = DiskManager::GetInstance();
    Disk disk("disk-8-204", SIZE, "/dev/block/disk-8-204", USB_FLAG);
    disk.SetExtraInfo(R"({"vid":"0781","pid":"5581","serialNumber":"ABC123","busnum":"2","devnum":"3"})");
    dm.OnDiskCreated(disk);
 
    std::string usbInfo;
    EXPECT_EQ(dm.GetUsbDeviceInfo("disk-8-204", usbInfo), E_OK);
    EXPECT_NE(usbInfo.find("\"vid\":\"0781\""), std::string::npos);
    EXPECT_NE(usbInfo.find("\"pid\":\"5581\""), std::string::npos);
    EXPECT_NE(usbInfo.find("\"serialNumber\":\"ABC123\""), std::string::npos);
    EXPECT_NE(usbInfo.find("\"busnum\":\"2\""), std::string::npos);
    EXPECT_NE(usbInfo.find("\"devnum\":\"3\""), std::string::npos);
 
    dm.OnDiskDestroyed("disk-8-204");
    GTEST_LOG_(INFO) << "GetUsbDeviceInfo_Success_TestCase_006 End";
}
} // namespace DiskManager
} // namespace OHOS