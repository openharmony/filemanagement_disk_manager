                                                 ...
                                                 const std::string &diskId)
{
    ...
    const ErrCode ret = storageDaemon_->GetBlockInfoByType(type, diskId, blockInfos);
    LOGI("GetBlockInfoByType exit ret=%{public}d bytes=%{public}zu", static_cast<int32_t>(ret), blockInfos.size());
...
int32_t StorageDaemonAdapter::CreateDmCryptVolume(const std::string &cryptParam, const std::string &loopPath,
                                                  const std::string &mapperName)
{
    ...
    const int32_t ret = storageDaemon_->CreateDmCryptVolume(cryptParam, loopPath, mapperName);
